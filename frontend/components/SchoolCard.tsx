...
export const SchoolCard: React.FC<SchoolCardProps> = ({school, viewMode, onDelete, onEdit }) => {
	return (
		<div 
		...
		>
			<div className={viewMode === 'list' ? "flex flex-col sm:flex-row items-center justify-between gap-4" : "w-full"}>
				<div className={viewMode === 'list' ? "flex items-center space-x-4" : "flex flex-col items-center gap-2 mb-4 relative"}>
					<span className={`font-bold text-[var(--safari-green)] ${viewMode === 'list' ? 'text-lg' : 'text-xl'}`}>{school.name}</span>
				</div>
        <div className={viewMode === 'list' ? "flex items-center gap-2 ml-auto" : "w-full mt-1 flex flex-col gap-2"}>
          <Button
