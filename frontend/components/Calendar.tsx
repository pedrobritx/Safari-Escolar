		...
		for (let i = 1; i <= daysInMonth; i++) {
			...
			days.push(
				<button
					...
					className={`h-10 w-10 md:h-12 md:w-12 rounded-[var(--radius-inner)] flex flex-col items-center justify-center text-sm font-bold transition-all relative
                    ${
											isSelected
		...
		<div className="card p-4 md:p-6 w-full h-full flex flex-col bg-[var(--surface-raised)] rounded-[var(--radius-outer)] border border-[var(--safari-stone-200)] shadow-sm">
			<div className="flex items-center justify-between mb-6">
				<button
					...
					className="p-2 rounded-full hover:bg-[var(--safari-stone-100)] text-[var(--safari-stone-600)] transition-colors"
				>
					<ChevronLeft size={20} />
				</button>
