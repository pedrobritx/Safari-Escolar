			...
			<form onSubmit={handleSubmit} className="p-6 space-y-6">
				...
				<div className="flex flex-col items-center gap-2">
					...
					<div className="relative">
						<button
							...
							className="cursor-pointer w-24 h-24 text-5xl flex items-center justify-center rounded-full border-4 border-[var(--safari-green-light)] shadow-sm hover:scale-105 transition-transform bg-white"
						...
						<div className="absolute -bottom-2 -right-2 bg-white rounded-full p-1.5 shadow-md border border-[var(--safari-stone-200)] pointer-events-none">
							<span className="text-xs font-bold text-[var(--safari-green)]">
								Mudar
							</span>
						</div>
					</div>
				...
				<div className="flex gap-3 pt-2">
					{mode === "edit" && onDelete && (
						<Button
							type="button"
							variant="accent" // Assuming accent is mapped to orange/danger in Button.tsx or globals
							onClick={() => {
								if (
									confirm(
					...
					<Button type="submit" variant="primary" className="flex-1 px-6 py-3">
						<Save size={20} />
