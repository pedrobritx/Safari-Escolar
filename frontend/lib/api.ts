...
export const api = {
	...
	async updateStudent(
		token: string,
		studentId: string,
		data: { name?: string; animalAvatar?: string; avatarColor?: string },
	) {
		return handleResponse(
	...
	async getTeachers(token: string) {
		return handleResponse(
			fetch(`${API_URL}/api/users/teachers`, {
				headers: {
					Authorization: `Bearer ${token}`,
				},
				cache: "no-store",
	...
	async getCoordinators(token: string) {
		return handleResponse(
			fetch(`${API_URL}/api/users/coordinators`, {
				headers: {
					Authorization: `Bearer ${token}`,
				},
				cache: "no-store",
	...
	async saveSchool(
		token: string,
		body: { id?: string; name: string; coordinatorId: string },
	) {
