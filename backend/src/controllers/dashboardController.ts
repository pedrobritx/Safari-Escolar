...
export const getDashboard = async (req: AuthRequest, res: Response) => {
	try {
		...
		if (["TEACHER", "COORDINATOR"].includes(userRole)) {
			where = {
				OR: [{ teacherId: userId }, { school: { coordinatorId: userId } }],
			};
		}
		...
		const dashboardData = classes.map((cls) => {
			...
			const todayFeedbackEvents = cls.students.flatMap(
				(student) => student.feedbackEvents,
			);
			...
			return {
				...
				todayNegativeEvents: todayFeedbackEvents.filter(
					(e) => e.type === "NEGATIVE",
				).length,
			};
		});
