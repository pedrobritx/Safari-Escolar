...
export const exportStudents = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // CSV Header
    const headers = ['Nome', 'Turma', 'Avatar', 'Total Presenças', 'Total Faltas', 'Total Atrasos', 'Taxa Presença (%)', 'Feedback Positivo', 'Feedback Negativo', 'Score Total'];
    ...
    for (const cls of classes) {
      for (const student of cls.students) {
        ...
        const positive = student.feedbackEvents.filter(e => e.type === 'positive').length;
        const negative = student.feedbackEvents.filter(e => e.type === 'negative').length;
    ...
    for (const cls of classes) {
      for (const student of cls.students) {
        ...
        for (const fb of student.feedbackEvents) {
          ...
          const typeLabel = fb.type === 'positive' ? 'Feedback Positivo' : 'Feedback Construtivo';
          const row = [
            dateStr,
            escapeCSV(student.name),
            escapeCSV(cls.name),
